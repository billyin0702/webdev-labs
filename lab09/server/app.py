...
async def respond_to_message(websocket, message):
    ...
    ********************************************************************/
    ...
    if data.get('type') != 'login' and data.get('type') != 'disconnect' and data.get('type') != 'chat':
        print('Unrecognized message type:', data)
        return
    
    # When the user logs in, add them to the logged_in_users dictionary
    if data.get('type') == 'login':
        logged_in_users[websocket] = data.get('username')
        
        ### Send the values of the logged_in_users dictionary to the client
        data['user_joined'] = data.get('username')
        data['active_users'] = list(logged_in_users.values())
    
    # When the user disconnects, remove them from the logged_in_users dictionary
    if data.get('type') == 'disconnect':
        del logged_in_users[websocket]
        ### Send the values of the logged_in_users dictionary to the client after removing the user
        data['user_left'] = data.get('username')
        data['active_users'] = list(logged_in_users.values())
