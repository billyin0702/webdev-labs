...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        console.log(data);
        document.querySelector('#users-list').innerHTML = `${data.active_users.join('<br>')}`;
        document.querySelector('#update').innerHTML = `${data.username} has joined the chat.`;
    } else if (data.type === "disconnect") {
        console.log('A user has just disconnected:');
        console.log(data);
        document.querySelector('#users-list').innerHTML = `${data.active_users.join('<br>')}`;
        document.querySelector('#update').innerHTML = `${data.username} has left the chat.`;
    } else if (data.type === "chat") {
        console.log('A user has just sent a chat message:');
        console.log(data);

        if (data.username === username) {
            document.querySelector('#chat').insertAdjacentHTML('beforeend', `<div class="each_text right">${data.text} <b>You:&ensp;</b> </div>`);
        }
        else{
            document.querySelector('#chat').insertAdjacentHTML('beforeend', `<div class="each_text left"><b>${data.username}:&ensp;</b> ${data.text} </div>`);
        }
